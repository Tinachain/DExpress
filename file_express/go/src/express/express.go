...
//export OnProgress
func OnProgress(express_handle int, param Object, file_path string, max int, cur int) bool {

	if gExpressClient != nil {
		return gExpressClient.ProgressEvent(express_handle, param, file_path, max, cur)
	}
...
//export OnFinish
func OnFinish(express_handle int, param Object, file_path string, size int64) {

	if gExpressClient != nil {
		gExpressClient.FinishEvent(express_handle, param, file_path, size)
	}
}
...
type ProgressCallBack func(express_handle int, param Object, file_path string, max int, cur int) bool
type FinishCallBack func(express_handle int, param Object, file_path string, size int64)
...
func (c *ExpressClient) SendFile(file_path string, save_relative_path string) int {
	...
	ret := C.send_file(C.int(c.expressHandle), nil, C.CString(file_path), C.CString(save_relative_path))
	resultInt := (*int)(unsafe.Pointer(&ret))
	return *resultInt
}

func (c *ExpressClient) SendDir(file_path string, save_relative_path string) int {

	if c.expressHandle <= 0 {
		return -1
	}

	ret := C.send_dir(C.int(c.expressHandle), nil, C.CString(file_path), C.CString(save_relative_path))
	resultInt := (*int)(unsafe.Pointer(&ret))
	return *resultInt
}
...
func (c *ExpressClient) ProgressEvent(express_handle int, param Object, file_path string, max int, cur int) bool {

	if c.HandleProgress != nil {
		return c.HandleProgress(express_handle, param, file_path, max, cur)
	}
	return true
}

func (c *ExpressClient) FinishEvent(express_handle int, param Object, file_path string, size int64) {

	if c.HandleFinish != nil {
		c.HandleFinish(express_handle, param, file_path, size)
	}
}
