...
bool OnProgress(int express_handle, void *param, char* file_path, int max, int cur);
void OnFinish(int express_handle, void *param, char* file_path, long long size);
...
bool send_file(int express_handle, void *param, char* file_path, char* save_relative_path)
{
    if(NULL != send_file_ptr)
	{
		return send_file_ptr(express_handle, param, file_path, save_relative_path);
	}
	return false;
}

bool send_dir(int express_handle, void *param, char* dir_path, char* save_relative_path)
{
    if(NULL != send_dir_ptr)
	{
		return send_dir_ptr(express_handle, param, dir_path, save_relative_path);
	}
	return false;
}

void stop_send_file(int express_handle, void *param, char* file_path)
{
    if(NULL != stop_send_file_ptr)
	{
		stop_send_file_ptr(express_handle, param, file_path);
	}
}
